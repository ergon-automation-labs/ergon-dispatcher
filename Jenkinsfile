pipeline {
  ...
  environment {
    BOT_NAME = 'dispatcher_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-dispatcher"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/dispatcher_bot/bin/dispatcher_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyDispatcher.Release.create()'"
          $RELEASE_BIN eval 'BotArmyDispatcher.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyDispatcher.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyDispatcher.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_dispatcher/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_dispatcher/releases/start_erl.data)
        fi
