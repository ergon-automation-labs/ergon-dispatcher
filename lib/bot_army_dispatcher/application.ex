defmodule BotArmyDispatcher.Application do
  ...
  def start(_type, _args) do
    children =
      ...
      |> maybe_add_bridge_health_monitor()
      |> maybe_add_log_error_scanner()
      |> maybe_add_daily_briefing_orchestrator()
      |> maybe_add_daily_reflection_orchestrator()
      ...
      |> maybe_add_wife_care_reminder_scheduler()
  ...
  defp maybe_add_bridge_health_monitor(children) do
    if env() == :test,
      do: children,
      else: [{BotArmyDispatcher.BridgeHealthMonitor, []} | children]
  end
  ...
  defp maybe_add_daily_reflection_orchestrator(children) do
    if env() == :test,
      do: children,
      else: [{BotArmyDispatcher.DailyReflectionOrchestrator, []} | children]
  end
  ...
  defp maybe_add_learning_feedback_scheduler(children) do
    if env() == :test,
      do: children,
      else: [{BotArmyDispatcher.LearningFeedbackScheduler, []} | children]
  end
  ...
  defp maybe_add_wife_care_reminder_scheduler(children) do
    if env() == :test,
      do: children,
      else: [{BotArmyDispatcher.WifeCareReminderScheduler, []} | children]
  end
